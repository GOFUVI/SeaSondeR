...
out$dual <- list(bearing = bearings[dual_peaks[,2]], a = seasonder_apm_obj[,dual_peaks[,2]])
...
seasonder_computePowerMatrix <- function(C,eig,a){
  ...
  # Schmit 1986 eqn 7
  l_min <- max(eig$values[1],0)
  a_star <- Conj(t(a))
  S0 <- l_min * eye(3)
  aa_inv <- solve(a_star %*% a)
  P <- aa_inv %*% a_star %*% (C-S0) %*% a %*% aa_inv
