...
#Detalles: Se deben escribir tests nuevos desde cero para la función auxiliar seasonder_CSSY2CSData. Estos tests deben validar que la función transforma correctamente la estructura del body del fichero CSSY en una lista de matrices con los campos obligatorios (SSA1, SSA2, SSA3, CS12, CS13, CS23, QC), en que cada fila de cada matriz corresponde al valor de cell$indx$index. Se deben incluir pruebas con una única celda y con múltiples celdas para verificar la correcta asignación y formación de números complejos a partir de las partes reales (c12r, c13r, c23r) y las partes imaginarias (c12i, c13i, c23i). Esta especificación se basa en los requerimientos documentados en la validación de la estructura en seasonder_validateCSDataStructure citeturn0file9.
#
library(testthat)
