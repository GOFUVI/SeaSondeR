...
\
La clase **`SeaSondeRCS`** es una representación en R de los archivos CS (Cross-spectra) de SeaSonde. Esta clase facilita la lectura, manipulación y análisis de los datos contenidos en estos archivos, proporcionando una estructura coherente y fácil de usar para los usuarios que trabajan con datos de SeaSonde en R.

## **Propósito**

\
El propósito principal de la clase **`SeaSondeRCS`** es proporcionar una interfaz unificada para trabajar con archivos CS en R. Esto incluye la capacidad de leer archivos CS, acceder a sus metadatos y datos, y realizar análisis específicos relacionados con los datos de SeaSonde. Al encapsular estos datos en una clase específica, se simplifica el proceso de trabajar con estos archivos y se garantiza una mayor consistencia y fiabilidad en los análisis.

## **Ejemplo de creación**

\
Para crear una instancia de la clase **`SeaSondeRCS`**, puedes utilizar el siguiente código en R:
...
# Crear un objeto SeaSondeRCS desde un archivo CS
archivo_CS <- "ruta/del/archivo/CSQ_o_CSS_o_CSA"
specs_path <- "ruta/del/archivo/especificaciones.yaml" # Si es necesario
objeto_SeaSondeRCS <- seasonder_createSeaSondeRCS(x = archivo_CS, specs_path = specs_path)
