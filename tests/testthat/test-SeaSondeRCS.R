...
test_that("El fichero exportado coincide con el fichero de referencia", {
  ...
  radial_metrics <- seasonder_exportLLUVRadialMetrics(
    seasonder_cs_obj, 
    LLUV_path = "tools/CSS_TORA_24_04_04_0700.ruv", 
    AngSeg = AngSeg
  )
  
  # Leer fichero de referencia instalado en el paquete
  expected_metrics <- data.table::fread(
    system.file("css_data/CSS_TORA_24_04_04_0700.ruv", package = "SeaSondeR"),
    header = TRUE
  )
  
  # Comparar los datos exportados con el fichero de referencia
  expect_equal(radial_metrics, expected_metrics, tolerance = 1e-6)
})
